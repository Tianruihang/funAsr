...
class SileroVAD(VAD):
    ...
    def is_vad(self, data):
        try:
            ...
            return None
    def is_unity_vad(self, data):
        try:
            audio_float32 = np.frombuffer(data, dtype=np.float32)

            # debug 范围
            # print(f"🧪 VAD 输入: shape={audio_float32.shape}, min={audio_float32.min()}, max={audio_float32.max()}")

            # 检查是否是正确的 shape
            if audio_float32.shape[-1] != 512:
                print(f"❌ 非法帧长度: {audio_float32.shape}")
                return None

            vad_output = self.vad_iterator(torch.from_numpy(audio_float32))
            if vad_output is not None:
                print(f"🟢 VAD 输出: {vad_output}")
            return vad_output
        except Exception as e:
            print(f"❌ VAD 处理异常: {e}")
            return None
